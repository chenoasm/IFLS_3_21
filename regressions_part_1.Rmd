...
h1_data <- clean %>% select(employed2, job_cat_H, jl_2, age, age_H, age_sq, age_sq_H, dl06, dependents, working_dependents, other_HHM, other_working, sc_code, wave, wage_prof)
           

p1_data <- h1_data %>% select(- wage_prof)

#probit1 based on glm6b
probit1 <- glm(employed2 ~ . , family = binomial (link = "probit"), data = p1_data)
summary(probit1)

mills <- invMillsRatio(probit1)$IMR1

heckit1 <- lm(tk22a ~ . - employed2 - job_cat_H - dependents + mills, data = p1_data[ p1_data$lfp == 1, ] )
