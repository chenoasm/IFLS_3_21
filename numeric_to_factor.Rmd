...
#covert tk01 and tk46m back to characters to resolve any merge issues
...
for (i in 1:length(names(IFLS4))){
  if(class(IFLS4[,names(IFLS4)[i]]) == "factor") {
    if (sum(levels(IFLS4[,names(IFLS4)[i]]) == levels(IFLS4[,names(IFLS5)[i]])) != length(levels(IFLS4[,names(IFLS5)[i]]))){
...
#relabel vars >>>> YIKES! FIX THIS!
names(IFLS4)[] <- c("pid", "hhid", "hhid_9", "hhid_9.y", "sc_code")
names(IFLS5)[] <- c("pid", "hhid", "hhid_9", "hhid_9.y", "sc_code")
