...
for (i in 1:length(names(IFLS4))){
  if(class(IFLS4[,names(IFLS4)[i]]) == "factor") {
    if(length(levels(IFLS4[,names(IFLS4)[i]])) != length(levels(IFLS5[,names(IFLS5)[i]]))){
      issues <- c(issues, names(IFLS4)[i])}
    else if (sum(levels(IFLS4[,names(IFLS4)[i]]) == levels(IFLS5[,names(IFLS5)[i]])) != length(levels(IFLS4[,names(IFLS4)[i]]))){
    issues <- c(issues, names(IFLS4)[i])
  }}
}
